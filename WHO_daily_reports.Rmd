...
```
```{r}
WHO_us <- WHO %>% filter(Country=="US")
WHO_world <- WHO %>% filter(Country !="US")
...
```{r}
# 
WHO_total_country_100 %>% ggplot() + geom_col(aes(x=reorder(Country,TotalCases),y=TotalCases)) +
        labs(title ="Countries with Confirmed Cases > 100") +
  coord_flip()
```
```{r}
#
WHO_total_country_500 %>% ggplot() + geom_col(aes(x=reorder(Country,TotalCases),y=TotalCases)) +
